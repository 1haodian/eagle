...
public class AlertBoltFunctionTest {
  ...
  private static class PrintinDoFn1 extends DoFn<KV<String, AlertStreamEvent>, String> {
    ...
    @ProcessElement public void processElement(ProcessContext c) {
      ...
      Assert.assertEquals(
          "KV{testAlertPublish1, Alert {site=yhd, stream=testAlertStream,timestamp=1970-01-01 00:00:00,001,data={jobId=140648764-oozie-oozi-W2017-06-05 04:56:28, visitCount=1, ip=yyy.yyy.yyy.yyy, operation=start}, policyId=policy4, createdBy=StreamPartition[streamId=oozieStream,type=GROUPBY,columns=[operation],sortSpec=[StreamSortSpec[windowPeriod=PT4S,windowMargin=1000]]]yhd, metaVersion=null}}",
          c.element().toString());
    }
  }
  ...
  private static class PrintinDoFn2 extends DoFn<KV<String, AlertStreamEvent>, String> {
    ...
    @ProcessElement public void processElement(ProcessContext c) {
      ...
      Assert.assertEquals(
          "KV{testAlertPublish1, Alert {site=yhd, stream=testAlertStream,timestamp=1970-01-01 00:00:00,013,data={jobId=140648764-oozie-oozi-W2017-06-05 04:56:28, visitCount=2, ip=yyy.yyy.yyy.yyy, operation=start}, policyId=policy4, createdBy=StreamPartition[streamId=oozieStream,type=GROUPBY,columns=[operation],sortSpec=[StreamSortSpec[windowPeriod=PT4S,windowMargin=1000]]]yhd, metaVersion=null}}",
          c.element().toString());
    }
  }
}
