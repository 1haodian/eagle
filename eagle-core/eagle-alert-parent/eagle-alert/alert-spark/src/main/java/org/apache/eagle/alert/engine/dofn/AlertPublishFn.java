...
public class AlertPublishFn
    ...
    for (Map.Entry<String, PolicyDefinition> entry : policiesMap.entrySet()) {
      try {
        ...
        if (entry.getValue().getDeduplication() != null) {
          this.deduplicatorMap
              .put(entry.getKey(), new DefaultDeduplicator(entry.getValue().getDeduplication()));
        }
  ...
  private class AlertContextEnrichFilter implements AlertStreamFilter {
    ...
    @Override public AlertStreamEvent filter(AlertStreamEvent event) {
      event.ensureAlertId();
