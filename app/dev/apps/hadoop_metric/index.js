...
(function () {
	...
	hadoopMetricApp.service("METRIC", function ($q, $http, Time, Site, Application) {
		...
		METRIC.QUERY_METRICS = '${baseURL}/rest/entities?query=GenericMetricService[${condition}]{*}&metricName=${metric}&pageSize=${limit}';
		/**
		...
		METRIC.metricsToSeries = function (name, metrics, rawData, option) {
		...
		METRIC.metrics = function (condition, metric, limit) {
			var config = {
